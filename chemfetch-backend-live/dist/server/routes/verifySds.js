...
router.use('/', createProxyMiddleware({
    ...
    onProxyReq: (proxyReq, req, res) => {
        console.log('[SDS Verify Proxy ▶︎ Python]', new Date().toISOString(), 'method:', proxyReq.method, 'path:', proxyReq.path, 'content-type:', proxyReq.getHeader('content-type'), 'content-length:', proxyReq.getHeader('content-length'));
    },
    onProxyRes: (proxyRes, req, res) => {
        console.log('[SDS Verify Python ◀︎ Proxy]', new Date().toISOString(), 'status:', proxyRes.statusCode, 'headers:', proxyRes.headers);
    },
    headers: {
