...
import { fetchBingLinks, scrapeProductInfo, fetchSdsByName, searchWithManualData, } from '../utils/scraper.js';
...
function normaliseUrl(u) {
    ...
    if (s.includes('dummy.local'))
        return null; // kill placeholder host
    return s;
}
// --- Routes ------------------------------------------------------------------
...
router.post('/', async (req, res) => {
    ...
    logger.info({ code, userId, name, size, isManualEntry, timestamp: Date.now() }, '[SCAN] Processing scan request');
    try {
        ...
        if (userId) {
            ...
            if (!productError && product) {
                ...
                if (!watchlistError && watchlistItem) {
                    logger.info({ code, productId: product.id, userId }, '[SCAN] Item already in user watchlist');
                    return res.json({
                        code,
        ...
        const { data: existing, error: fetchErr } = await supabase
            ...
            .eq('barcode', code)
            .maybeSingle();
        logger.info({ code, dbLookupTime: Date.now() - dbLookupStart }, '[SCAN] Database lookup completed');
        if (fetchErr)
        ...
        if (existing) {
            ...
            if (!existing.sds_url && existing.name) {
                try {
                    ...
                    const { sdsUrl: foundSds } = await fetchSdsByName(existing.name, existing.contents_size_weight || undefined);
            ...
            logger.info({ code, updated }, '[SCAN] Returning existing product from database (no scraping)');
            return res.json({
                code,
                product: updated,
        ...
        if (isManualEntry) {
            ...
            logger.info({ code, rawUrls: urls, searchTime: Date.now() - searchStart }, '[SCAN] Raw URLs from search');
            const cleaned = [...new Set((urls || []).map(normaliseUrl).filter(Boolean))];
            ...
            for (const u of cleaned.slice(0, 5)) {
                if (foundGoodResult)
                    break; // Stop when we find a good result
                const startTime = Date.now();
                try {
                    const r = (await scrapeProductInfo(u, code));
        ...
        if (!isManualEntry && !top.sdsUrl && top.name) {
            try {
                ...
                const { sdsUrl: fallbackSds, topLinks } = await fetchSdsByName(top.name, top.size ?? top.contents_size_weight);
                const sdsSearchTime = Date.now() - sdsSearchStart;
                logger.info({ name: top.name, fallbackSds, topLinks, sdsSearchTime }, '[SCAN] Fallback SDS result');
                if (fallbackSds)
                    top.sdsUrl = fallbackSds;
            }
            catch (err) {
