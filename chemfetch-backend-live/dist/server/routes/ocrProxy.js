...
const proxyOptions = {
    ...
    onProxyReq: (proxyReq, req, res) => {
        console.log('[OCR Proxy ▶︎ Python]', new Date().toISOString(), 'method:', proxyReq.method, 'path:', proxyReq.path, 'content-type:', proxyReq.getHeader('content-type'), 'content-length:', proxyReq.getHeader('content-length'));
    },
    onProxyRes: (proxyRes, req, res) => {
        console.log('[OCR Python ◀︎ Proxy]', new Date().toISOString(), 'status:', proxyRes.statusCode, 'headers:', proxyRes.headers);
    },
    headers: {
