...
const SIZE_PATTERN = /\b\d+(?:[.,]\d+)?\s?(?:ml|mL|l|L|litre|liter|g|kg|oz|fl\s?oz|pack|packs|tablet|tablets)\b/i;
...
function isLikelySds(url: string, anchorText?: string): boolean {
 ...
 if (lowerUrl.endsWith(".pdf") && (lowerUrl.includes("sds") || lowerUrl.includes("msds") || lowerText.includes("sds") || lowerText.includes("safety data"))) {
 return true;
 }
 if (lowerUrl.includes("sds") || lowerUrl.includes("msds") || lowerUrl.includes("safety-data-sheet")) {
 return true;
 }
 if (lowerText.includes("safety data") || lowerText.includes("sds") || lowerText.includes("msds")) {
 return true;
 }
 return false;
}
...
export async function scrapeProductInfo(url: string, barcode?: string): Promise<ProductScrapeResult> {
 const normalised = normaliseLink(url);
 if (!normalised) {
...
export async function fetchSdsByName(name: string, size?: string): Promise<{ sdsUrl: string | null; topLinks: string[] }> {
...
export async function fetchSdsByNameSimple(name: string, size?: string): Promise<{ sdsUrl: string | null; topLinks: string[] }> {
 return fetchSdsByName(name, size);
}

export async function searchWithManualData(name: string, size: string, code: string): Promise<ProductScrapeResult> {
 const cleanedName = cleanText(name);
 const cleanedSize = cleanText(size);
